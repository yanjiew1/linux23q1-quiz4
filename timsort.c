                            ...
                            bool (*cmp)(const void *, const void *))
{
	...
	__reverse(cur_1, cur_2, size); // AAAA
...
static inline bool merge_rule(run_t *stack, size_t top)
{
    ...
    return run_length(stack[top]) > run_length(stack[top - 1]) ||
           run_length(stack[top]) + run_length(stack[top - 1]) > // BBBB
               run_length(stack[top - 2]); // CCCC
}
                                    ...
                                    bool (*cmp)(const void *, const void *))
{
    ...
    if (next < last && run_len < MIN_RUN) {
        last = first + MIN_RUN * size < last ? first + MIN_RUN * size : last; // DDDD
             ...
             bool (*cmp)(const void *, const void *))
{
    ...
    while (top > 1) {
        ...
        stack[top - 1].last = stack[top].last; // GGGG
    }
